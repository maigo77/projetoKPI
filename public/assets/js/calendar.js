...
(function ( $ ) {
    ...
    // week names
    var weekNames = ["Domingo", "Segunda-feira", "Terça-feira", "Quarta-feira", "Quinta-feira", "Sexta-feira", "Sábado"];
      ...
      return weeks;
		...
		*/
		var draw = function() {
          ...
          var prevLinkGroup = $("<i id='dncalendar-prev-month' class='fas fa-arrow-circle-left dncalendar-prev-month'></i>");
          var nextLinkGroup = $("<i id='dncalendar-next-month' class='fas fa-arrow-circle-right dncalendar-next-month'></i>");
          ...
          for (var i = 0; i < nDates; i++) {
            ...
            for (var j = 0; j < nDate; j++) {
              ...
              var controleDeClasses = false
              var controleDeMeses   = false
            
              if(dataHoje < date && mesAtual == month){
                  showCalendarClick = true
                  controleDeClasses = true
              }else if(month == mesAtual + 1 || month == mesAtual + 2){
                  showCalendarClick = true
                  controleDeClasses = true
              }else{
                  showCalendarClick = false
              ...
              if(month == parseInt(mesAtual - 1) || month == parseInt(mesAtual + 2)){
                  controleDeMeses   = true
              }

              var colDate = "<td id='"+ ((showCalendarClick == true) ? 'calendarClick' : '') +"' class='"+ colDateClass +" "+((controleDeMeses == true) ? 'numApagado' : '') +" "+ ((showCalendarClick == true) ? 'calendarClick' : '') +" "+ (controleDeClasses == true ? 'disponivel' : 'indisponivel') +"' data-date='"+ date +"' data-month='"+ month +"' data-year='"+ year +"'><div class='entry'"+ colDateDataAttr +"'>"+ date +"</div></td>";
    ...
    var getNotesThisMonth = function() {
          ...
          return result;
            ...
            var verificaDiasDisponiveis = function(){
                  let arrDatas = JSON.parse(document.querySelector('#arrDatas').value)
		...
		var nextMonth = function() {
			...
			var year = firstDateOfNextMonth.getFullYear();
                  ...
                  document.querySelector('#confirma-folga').classList.add('dNone')
        ...
        var limitaMeses = (operador, range) => {
          var mesAtual = new Date().getMonth();
          var mesSelecionado = parseInt($(".dncalendar-body table tbody tr td:eq(7)").attr('data-month'))

          if(operador == '+'){
            var limite = range + mesAtual
            limite = limite > 12 ? 12 : limite

            if(mesSelecionado > limite){
              return false
            }else{
              return true
            }
